 ...
 */
function paragraphBlockRange(view) {
    const { state } = view;
...
export function canConvertSelectionToTable(view) {
...
export function convertSelectionToTable(view) {
    ...
    while (lines.length > 0 && ((_a = lines[lines.length - 1]) === null || _a === void 0 ? void 0 : _a.trim()) === '')
        lines.pop();
    while (lines.length > 0 && ((_b = lines[0]) === null || _b === void 0 ? void 0 : _b.trim()) === '')
        lines.shift();
    if (lines.length === 0)
        return false;
    ...
    const rows = cellRows.map((cells) => {
        const cellNodes = cells.map((text) => {
            const para = text.length > 0 ? paragraphType.create({}, schema.text(text)) : paragraphType.create();
...
// ---------------------------------------------------------------------------
export function canConvertTableToText(view) {
 ...
 */
export function convertTableToText(view) {
    const { state } = view;
    const ctx = getTableContext(state);
    if (!ctx.isInTable || !ctx.table || ctx.tablePos === undefined)
        return false;
    ...
    table.forEach((row) => {
        ...
        paragraphs.push(joined.length > 0 ? paragraphType.create({}, schema.text(joined)) : paragraphType.create());
    });
    if (paragraphs.length === 0)
        return false;
...
//# sourceMappingURL=convertTextToTable.js.map