...
function resolveLanguageName(input) {
    ...
    if (COMMON_LANGS[lower])
        return COMMON_LANGS[lower];
    // Already a language name — title-case it.
    return trimmed.charAt(0).toUpperCase() + trimmed.slice(1);
}
export const translateRangeTool = {
    name: 'translate',
    ...
    execute(args, ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const semanticTranslateValidator = (o) => {
                var _a;
                const t = ((_a = o.translation) !== null && _a !== void 0 ? _a : '').trim();
                if (!t)
                    return [{ field: 'translation', text: '', reason: 'is empty' }];
                if (t === selection.trim())
                    return [
                        {
                            field: 'translation',
            ...
            try {
                out = yield runJsonChatWithValidation([
                    ...
                    temperature: 0.2,
                    signal: ctx.signal,
                    validator: combineValidators(noPlaceholderValidator, semanticTranslateValidator),
                });
            }
            catch (err) {
                return {
            ...
            const paragraphs = translation
                ...
                .map((p) => p.trim())
                .filter(Boolean);
            const fragment = Fragment.fromArray(paragraphs.map((p) => paraType.create(null, ctx.schema.text(p))));
...
//# sourceMappingURL=translateRange.js.map