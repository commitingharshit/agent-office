...
const TONE_HINTS = {
    ...
    readable: 'Improve readability: break sentences longer than 25 words into two; replace abstract or jargon-heavy phrases with concrete plain English; prefer active voice; aim for a Flesch-Kincaid grade level around 8. Keep the meaning unchanged.',
    plain: 'Rewrite in plain English suitable for a general audience: short sentences (under 20 words), simple words, active voice, no jargon. Keep the meaning intact.',
};
export const applyRewriteTool = {
    name: 'rewrite',
    ...
    execute(args, ctx) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const extra = (_a = args.instruction) === null || _a === void 0 ? void 0 : _a.trim();
...
Return ONLY a JSON object: {"rewrite": "<the rewritten text, plain prose, no markdown>"}.`;
            ...
            const semanticRewriteValidator = (o) => {
                var _a;
                const r = ((_a = o.rewrite) !== null && _a !== void 0 ? _a : '').trim();
                if (!r)
                    return [{ field: 'rewrite', text: '', reason: 'is empty' }];
                if (r === selection.trim())
                    return [
                        {
                            field: 'rewrite',
            ...
            let out;
            try {
                out = yield runJsonChatWithValidation([
                    ...
                    temperature: 0.4,
                    signal: ctx.signal,
                    validator: combineValidators(noPlaceholderValidator, semanticRewriteValidator),
                });
            }
            catch (err) {
                return { kind: 'error', message: `Rewrite failed — ${err.message}` };
            }
            const replacement = stripModelPreamble(out.rewrite);
            if (!replacement)
                return { kind: 'error', message: 'Model returned an empty rewrite.' };
            ...
            const paragraphs = replacement
                ...
                .map((p) => p.trim())
                .filter(Boolean);
            const replacementFragment = Fragment.fromArray(paragraphs.map((p) => paraType.create(null, ctx.schema.text(p))));
...
//# sourceMappingURL=applyRewrite.js.map