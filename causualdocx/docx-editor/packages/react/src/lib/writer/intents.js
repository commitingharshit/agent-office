...
export function classifyIntent(userMessage, ctx, signal) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const out = yield runJsonChat(messages, {
                schema: SCHEMA,
                maxTokens: 120,
                temperature: 0.1,
                signal,
...
function quickClassify(message, ctx) {
    var _a, _b;
    ...
    if (tableVerb.test(m) || /tabular\b/.test(m)) {
        ...
        const topic = m
            ...
            .trim();
        return Object.assign(Object.assign({ intent: 'insertTable', topic: topic || undefined }, (rows ? { rows } : {})), (cols ? { cols } : {}));
    }
    ...
    const transformVerb = /\b(create|make|build|turn\s+this\s+into|format|restructure|re-?write\s+this\s+as|draft)\b/;
    if (transformVerb.test(m)) {
        let transformTarget = null;
        if (/\b(?:[a-z][\w-]*\s+){0,4}?resume\b/.test(m))
            transformTarget = 'resume';
        else if (/\bcover[\s-]?letter\b/.test(m))
            transformTarget = 'cover-letter';
        ...
        else if (/\b(?:academic|research)\s+(?:paper|article)\b/.test(m) ||
            /\b(?:as\s+an?\s+)?academic\s+paper\b/.test(m) ||
            /\bin\s+(?:MLA|Chicago|APA)\s+(?:style|format)\b/i.test(m))
            transformTarget = 'academic';
        else if (/\b(?:slide\s+deck|presentation\s+outline|presentation|deck|slides?\s+outline)\b/.test(m) ||
        ...
        if (transformTarget) {
            ...
            const instruction = (_a = message.match(/\b(ATS[\s-]?(?:friendly|optimi[sz]ed)|one[\s-]?page|short|concise|long|detailed|formal|casual|MLA|APA|Chicago|SEO|BLUF|executive[\s-]?summary|10[\s\/-]?20[\s\/-]?30|Pecha\s*Kucha|Kawasaki)\b/i)) === null || _a === void 0 ? void 0 : _a[0];
            return Object.assign({ intent: 'transformDoc', transformTarget }, (instruction ? { instruction } : {}));
        }
    }
    ...
    const researchVerb = message
        .trim()
        .match(/^(?:what(?:'s| is)|who(?:'s| is)|tell me about|look up|search for|find info(?:rmation)? on|define)\s+(.+?)[?!.]*$/i);
    if (researchVerb) {
        const query = (_b = researchVerb[1]) === null || _b === void 0 ? void 0 : _b.trim();
        if (query)
            return { intent: 'research', query };
    }
    if (ctx.hasSelection && /^(rewrite|rephrase|polish|improve)\b/.test(m)) {
        ...
        const tone = afterVerb.match(/\b(concise|formal|casual|shorter|longer|readable|plain)(?:ly)?\b/);
        if (tone)
...
function pickInt(match) {
    var _a;
    if (!match)
        return undefined;
    const n = parseInt((_a = match[1]) !== null && _a !== void 0 ? _a : '', 10);
    return Number.isFinite(n) ? n : undefined;
}
function normaliseIntent(out, userMessage) {
