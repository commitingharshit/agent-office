...
export function useVisualLineNavigation({ pagesContainerRef }) {
    ...
    const getCaretClientX = useCallback((pmPos) => {
        ...
        for (const spanEl of spans) {
            ...
            if (pmPos >= pmStart && pmPos <= pmEnd && ((_a = spanEl.firstChild) === null || _a === void 0 ? void 0 : _a.nodeType) === Node.TEXT_NODE) {
     ...
     */
    const findLineElementAtPosition = useCallback((pmPos) => {
        if (!pagesContainerRef.current)
            return null;
     ...
     */
    const findPositionOnLineAtClientX = useCallback((lineEl, clientX) => {
        const spans = lineEl.querySelectorAll('span[data-pm-start][data-pm-end]');
        if (spans.length === 0) {
     ...
     */
    const handlePMKeyDown = useCallback((view, event) => {
        ...
        if (!pagesContainerRef.current)
            return false;
        const allLines = Array.from(pagesContainerRef.current.querySelectorAll(CONTENT_LINE_SELECTOR));
...
//# sourceMappingURL=useVisualLineNavigation.js.map