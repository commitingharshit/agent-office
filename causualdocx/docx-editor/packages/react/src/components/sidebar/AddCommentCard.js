...
export function AddCommentCard({ measureRef, onSubmit, onCancel, knownAuthors = [], }) {
    ...
    const handleSubmit = () => {
        if (text.trim()) {
            onSubmit === null || onSubmit === void 0 ? void 0 : onSubmit(text.trim());
            setText('');
            setCursorPos(0);
        }
    };
    ...
    return (_jsxs("div", { ref: measureRef, onMouseDown: (e) => e.stopPropagation(), style: {
            padding: 12,
            borderRadius: 8,
            ...
            zIndex: 50,
            position: 'relative',
        }, children: [_jsx("textarea", { ref: (el) => {
                    textareaRef.current = el;
                    el === null || el === void 0 ? void 0 : el.focus({ preventScroll: true });
                }, value: text, onChange: (e) => {
                    var _a;
                    setText(e.target.value);
                    setCursorPos((_a = e.target.selectionStart) !== null && _a !== void 0 ? _a : e.target.value.length);
                }, onSelect: (e) => {
                    var _a;
                    const ta = e.target;
                    setCursorPos((_a = ta.selectionStart) !== null && _a !== void 0 ? _a : text.length);
                }, onClick: (e) => {
                    var _a;
                    const ta = e.target;
                    setCursorPos((_a = ta.selectionStart) !== null && _a !== void 0 ? _a : text.length);
                }, onMouseDown: (e) => e.stopPropagation(), onKeyDown: handleKeyDown, onKeyUp: (e) => {
                    var _a;
                    const ta = e.target;
                    setCursorPos((_a = ta.selectionStart) !== null && _a !== void 0 ? _a : text.length);
                }, placeholder: t('comments.addComment'), style: {
                    width: '100%',
                ...
                } }), mention && suggestions.length > 0 && (_jsx("div", { role: "listbox", "aria-label": t('comments.mentionSuggestions'), style: dropdownStyle, onMouseDown: (e) => e.preventDefault(), "data-testid": "mention-suggestions", children: suggestions.map((name, idx) => (_jsx("div", { role: "option", "aria-selected": idx === activeMentionIdx, onMouseEnter: () => setActiveMentionIdx(idx), onClick: () => insertMention(name), style: dropdownItemStyle(idx === activeMentionIdx), "data-testid": `mention-option-${name}`, children: name }, name))) })), _jsxs("div", { style: { display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 8 }, children: [_jsx("button", { onClick: () => {
                            onCancel === null || onCancel === void 0 ? void 0 : onCancel();
                            setText('');
                        }, style: CANCEL_BUTTON_STYLE, children: t('common.cancel') }), _jsx("button", { onClick: handleSubmit, disabled: !text.trim(), style: submitButtonStyle(!!text.trim()), children: t('common.comment') })] })] }));
}
//# sourceMappingURL=AddCommentCard.js.map