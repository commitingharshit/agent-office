...
export const OUTLINE_BUTTON_RESERVED_SPACE = OUTLINE_BUTTON_LEFT_OFFSET + OUTLINE_BUTTON_BOX + OUTLINE_PAGE_GAP;
/**
...
function buildOutlineTree(headings) {
    const descendantsByIndex = headings.map(() => []);
...
export const DocumentOutline = React.memo(function DocumentOutline({ headings, onHeadingClick, onClose, topOffset = 0, scrollLeft = 0, activeIndex = null, }) {
    ...
    const { descendantsByIndex, parentIndices } = useMemo(() => buildOutlineTree(headings), [headings]);
    ...
    const hiddenByCollapse = useMemo(() => {
        ...
        for (const parent of collapsed) {
            if (parent >= headings.length)
                continue;
            for (const desc of (_a = descendantsByIndex[parent]) !== null && _a !== void 0 ? _a : [])
                hidden.add(desc);
        }
        return hidden;
    ...
    const handleHeadingActivate = (index, pmPos) => {
        var _a;
        if (hiddenByCollapse.has(index)) {
            const nextCollapsed = new Set(collapsed);
            // Expand every ancestor of this index.
            for (let i = index - 1; i >= 0; i--) {
                if (nextCollapsed.has(i) && ((_a = descendantsByIndex[i]) !== null && _a !== void 0 ? _a : []).includes(index)) {
    ...
    // existing heading.
    const headingsKey = useMemo(() => headings.map((h) => `${h.level}:${h.text}`).join('|'), [headings]);
    const lastResetKeyRef = React.useRef(headingsKey);
    useEffect(() => {
    ...
    return (_jsxs("nav", { className: "docx-outline-nav", role: "navigation", "aria-label": t('documentOutline.ariaLabel'), style: {
            position: 'absolute',
            top: topOffset,
            ...
            transition: 'transform var(--doc-anim-base)',
        }, onMouseDown: (e) => e.stopPropagation(), children: [_jsxs("div", { style: {
                    display: 'flex',
                    alignItems: 'center',
                    gap: 10,
                    padding: '14px 16px',
                    ...
                    flexShrink: 0,
                }, children: [_jsx(MaterialSymbol, { name: "format_list_bulleted", size: 18 }), _jsx("span", { style: {
                            flex: 1,
                            minWidth: 0,
                            whiteSpace: 'nowrap',
                            overflow: 'hidden',
                            ...
                            color: 'var(--doc-text)',
                        }, children: t('documentOutline.title') }), _jsx(Tooltip, { content: t('documentOutline.closeTitle'), children: _jsx("button", { onClick: onClose, "aria-label": t('documentOutline.closeAriaLabel'), style: {
                                border: 'none',
                    ...
                    return (_jsxs("div", { style: {
                            ...
                            marginLeft: heading.level * 10,
                        }, children: [isParent ? (_jsx("button", { type: "button", onClick: () => toggleCollapse(index), "aria-label": isCollapsed ? t('documentOutline.expand') : t('documentOutline.collapse'), "aria-expanded": !isCollapsed, style: {
                                    width: 16,
                                    height: 18,
                                    padding: 0,
                                    background: 'none',
                                ...
                                }, children: _jsx(MaterialSymbol, { name: "arrow_drop_down", size: 18 }) })) : (_jsx("span", { style: { width: 16, flexShrink: 0 }, "aria-hidden": "true" })), _jsx("button", { className: "docx-outline-heading-btn", onClick: () => handleHeadingActivate(index, heading.pmPos), "aria-current": isActive ? 'true' : undefined, style: {
                                    flex: 1,
                                    textAlign: 'left',
                                ...
                                }, children: heading.text })] }, `${heading.pmPos}-${index}`));
                })) })] }));
});
