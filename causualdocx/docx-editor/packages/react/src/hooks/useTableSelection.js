...
import { TableSelectionManager, getTableFromDocument, updateTableInDocument, deleteTableFromDocument, } from '@eigenpal/docx-core';
import { createTableContext, addRow, deleteRow, addColumn, deleteColumn, mergeCells, getTableSplitCellDialogConfig, splitTableCell, getColumnCount, } from '../components/ui/TableToolbar';
// ============================================================================
...
// HOOK
// ============================================================================
export function useTableSelection({ document: doc, onChange, onSelectionChange, }) {
    ...
    const handleCellClick = useCallback((tableIndex, rowIndex, columnIndex) => {
        if (!doc)
            return;
        ...
        const selection = { tableIndex, rowIndex, columnIndex };
        const context = createTableContext(table, selection);
        setState({ context, table, tableIndex, rowIndex, columnIndex });
        onSelectionChange === null || onSelectionChange === void 0 ? void 0 : onSelectionChange(context);
    ...
    const getSplitCellConfig = useCallback(() => {
    ...
    const applySplitCell = useCallback((rows, cols) => {
        if (!doc ||
            !state.table ||
        ...
        onChange === null || onChange === void 0 ? void 0 : onChange(newDoc);
        if (newDoc) {
    ...
    const isCellSelected = useCallback((tableIndex, rowIndex, columnIndex) => {
