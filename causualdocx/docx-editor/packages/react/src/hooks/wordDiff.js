...
function tokenise(text) {
    var _a;
    return (_a = text.match(TOKEN_RE)) !== null && _a !== void 0 ? _a : [];
}
...
export function diffWords(before, after) {
    ...
    if (!before)
        return after ? [{ op: 'add', text: after }] : [];
    if (!after)
        return [{ op: 'remove', text: before }];
...
export function diffStats(segments) {
    ...
    for (const s of segments) {
        if (s.op === 'add')
            added += countWords(s.text);
        else if (s.op === 'remove')
            removed += countWords(s.text);
    }
    return { added, removed };
}
//# sourceMappingURL=wordDiff.js.map