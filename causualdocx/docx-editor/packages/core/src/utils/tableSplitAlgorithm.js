...
// Column width math
// ---------------------------------------------------------------------------
export function sumColumnWidths(widths, start, span) {
    let total = 0;
 ...
 */
export function redistributeColumnWidths(existing, startCol, currentSpan, targetSpan) {
    ...
    const replacement = Array.from({ length: targetSpan }, (_, i) => segmentWidth + (i < remainder ? 1 : 0));
    return [
 ...
 */
export function computeSplitLayout(anchors, target, rows, cols, totalRows, createSplitCellData) {
    ...
    for (const anchor of anchors) {
        ...
        nextAnchors.push({
            ...
            rowspan: anchor.rowspan + (deltaRows > 0 && rowIntersectsBand && !colIntersectsBand ? deltaRows : 0),
            colspan: anchor.colspan + (deltaCols > 0 && colIntersectsBand && !rowIntersectsBand ? deltaCols : 0),
        });
    }
    // Add new split cells
 ...
 */
export function buildAnchorMaps(anchors) {
    const byStart = new Map();
    const byCoveredSlot = new Map();
    for (const anchor of anchors) {
 ...
 */
export function computeSplitDialogDefaults(rowspan, colspan) {
...
//# sourceMappingURL=tableSplitAlgorithm.js.map