 ...
 */
function parseModifierValue(hexValue) {
    if (!hexValue)
        return 1;
...
function applyTint(hex, tint) {
    ...
    return rgbToHex(Math.min(255, Math.max(0, Math.round(rgb.r * tint + 255 * (1 - tint)))), Math.min(255, Math.max(0, Math.round(rgb.g * tint + 255 * (1 - tint)))), Math.min(255, Math.max(0, Math.round(rgb.b * tint + 255 * (1 - tint)))));
}
/**
...
function applyShade(hex, shade) {
    ...
    const rgb = hexToRgb(hex);
    return rgbToHex(Math.min(255, Math.max(0, Math.round(rgb.r * shade))), Math.min(255, Math.max(0, Math.round(rgb.g * shade))), Math.min(255, Math.max(0, Math.round(rgb.b * shade))));
}
/**
 ...
 */
function getThemeColorValue(theme, slot) {
    var _a, _b, _c, _d;
    // Map alias slots to actual color scheme keys
    const schemeKey = (_a = THEME_COLOR_ALIASES[slot]) !== null && _a !== void 0 ? _a : slot;
    ...
    const isSchemeKey = (key) => schemeKeys.includes(key);
    if (!(theme === null || theme === void 0 ? void 0 : theme.colorScheme)) {
        if (isSchemeKey(schemeKey)) {
            return (_b = DEFAULT_THEME_COLORS[schemeKey]) !== null && _b !== void 0 ? _b : '000000';
        }
        return '000000';
    }
    if (isSchemeKey(schemeKey)) {
        return (_d = (_c = theme.colorScheme[schemeKey]) !== null && _c !== void 0 ? _c : DEFAULT_THEME_COLORS[schemeKey]) !== null && _d !== void 0 ? _d : '000000';
    }
    return '000000';
}
/**
 ...
 */
function resolveThemeColorSlot(colorName) {
    var _a;
    if (!colorName)
        return null;
    const normalized = colorName.toLowerCase();
    const slot = (_a = THEME_COLOR_ALIASES[colorName]) !== null && _a !== void 0 ? _a : THEME_COLOR_ALIASES[normalized];
 ...
 */
export function resolveColor(color, theme, defaultColor = '000000') {
    var _a;
    if (!color) {
        return `#${defaultColor}`;
    }
    ...
    if (color.themeColor) {
        ...
        if (slot) {
            ...
            hexColor = (_a = color.rgb) !== null && _a !== void 0 ? _a : defaultColor;
        }
 ...
 */
export function resolveColorToHex(color, theme) {
    if (!color || color.auto)
        return undefined;
 ...
 */
export function resolveHighlightColor(highlight) {
 ...
 */
export function resolveShadingColor(color, theme) {
    if (!color)
        return '';
 ...
 */
export function isBlack(color, theme) {
    if (!color)
        return false;
    if (color.auto)
        return true;
 ...
 */
export function isWhite(color, theme) {
    if (!color)
        return false;
    const resolved = resolveColor(color, theme);
    const hex = resolved.replace(/^#/, '').toLowerCase();
 ...
 */
export function getContrastingColor(backgroundColor, theme) {
    if (!backgroundColor)
        return '#000000';
 ...
 * @returns ColorValue object
 */
export function createThemeColor(themeColor, tint, shade) {
    const result = { themeColor };
 ...
 * @returns ColorValue object
 */
export function createRgbColor(hex) {
 ...
 * @returns CSS color string
 */
export function darkenColor(color, theme, percent) {
    const resolved = resolveColor(color, theme);
 ...
 * @returns CSS color string
 */
export function lightenColor(color, theme, percent) {
    const resolved = resolveColor(color, theme);
    const hex = resolved.replace(/^#/, '');
 ...
 * @returns CSS color string
 */
export function blendColors(color1, color2, ratio, theme) {
 ...
 */
export function resolveHighlightToCss(value) {
 ...
 * @returns 6-character hex color (no #)
 */
export function getThemeTintShadeHex(baseHex, type, fraction) {
    if (type === 'tint') {
 ...
 */
export function generateThemeTintShadeMatrix(colorScheme) {
    const scheme = colorScheme !== null && colorScheme !== void 0 ? colorScheme : DEFAULT_THEME_COLORS;
    return THEME_MATRIX_ROWS.map((row) => {
        return THEME_MATRIX_COLUMNS.map((col) => {
            var _a, _b;
            const baseHex = (_b = (_a = scheme[col.slot]) !== null && _a !== void 0 ? _a : DEFAULT_THEME_COLORS[col.slot]) !== null && _b !== void 0 ? _b : '000000';
            let hex;
            if (row.type === 'base') {
 ...
 */
export function colorsEqual(color1, color2, theme) {
    if (!color1 && !color2)
        return true;
    if (!color1 || !color2)
        return false;
