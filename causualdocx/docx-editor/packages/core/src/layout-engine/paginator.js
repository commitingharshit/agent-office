...
export function createPaginator(options) {
    var _a;
    let pageSize = Object.assign({}, options.pageSize);
    let margins = Object.assign({}, options.margins);
    let columns = (_a = options.columns) !== null && _a !== void 0 ? _a : { count: 1, gap: 0 };
     ...
     * explicit width.
     */
    function getColumnWidthAt(columnIndex) {
        var _a;
        const cw = columns.columnWidths;
        if (cw && cw.length === columns.count) {
            const w = (_a = cw[Math.min(columnIndex, columns.count - 1)]) === null || _a === void 0 ? void 0 : _a.width;
    ...
    function createNewPage() {
        ...
        const footnoteHeight = (_b = (_a = options.footnoteReservedHeights) === null || _a === void 0 ? void 0 : _a.get(pageNumber)) !== null && _b !== void 0 ? _b : 0;
        const pageContentBottom = contentBottom - footnoteHeight;
        const page = {
            number: pageNumber,
            fragments: [],
            margins: Object.assign({}, margins),
            size: Object.assign({}, pageSize),
            ...
            columns: columns.count > 1 ? Object.assign({}, columns) : undefined,
        };
        const state = {
            page,
            cursorY: topMargin,
        ...
        while (!fits(safeHeight, state)) {
            ...
            if (safeHeight > columnCapacity) {
                if (!warnedOversizedFragment) {
                    ...
                    console.warn(`Paginator: fragment height ${safeHeight.toFixed(0)}px exceeds page content height ${columnCapacity.toFixed(0)}px; placing with overflow.`);
                }
    ...
    function updateColumns(newColumns) {
        ...
        const state = getCurrentState();
        state.page.columns = columns.count > 1 ? Object.assign({}, columns) : undefined;
     ...
     */
    function ensureColumnRegionFits(regionHeight) {
        ...
        if (regionHeight <= available)
            return;
        if (regionHeight > fullColumnCapacity)
            return; // can't fit anywhere; let it split
        ...
        fresh.columnIndex = 0;
        fresh.page.columns = columns.count > 1 ? Object.assign({}, columns) : undefined;
    }
    /**
    ...
    function updatePageLayout(newPageSize, newMargins, applyImmediately = true) {
        if (!applyImmediately) {
            pendingPageSize = newPageSize ? Object.assign({}, newPageSize) : pendingPageSize;
            pendingMargins = newMargins ? Object.assign({}, newMargins) : pendingMargins;
            return;
        }
        if (newPageSize) {
            pageSize = Object.assign({}, newPageSize);
        }
        if (newMargins) {
            margins = Object.assign({}, newMargins);
        }
        if (getContentHeight() <= 0) {
