...
// ============================================================================
export function isHyperlinkActive(state) {
    const hlType = state.schema.marks.hyperlink;
    if (!hlType)
        return false;
    return isMarkActive(state, hlType);
}
export function getHyperlinkAttrs(state) {
    const hlType = state.schema.marks.hyperlink;
    if (!hlType)
        return null;
...
export const HyperlinkExtension = createMarkExtension({
    ...
    onSchemaReady(ctx) {
        ...
        const setHyperlink = (href, tooltip) => {
        ...
        const removeHyperlink = (state, dispatch) => {
            const { from, to, empty } = state.selection;
            if (empty) {
        ...
        const insertHyperlink = (text, href, tooltip) => {
            return (state, dispatch) => {
                if (dispatch) {
                    const mark = hlType.create({ href, tooltip: tooltip || null });
...
//# sourceMappingURL=HyperlinkExtension.js.map