 ...
 */
function isOpeningContext(prevChar) {
    if (prevChar === null || prevChar === '')
        return true; // start of block / no preceding char
    if (/[\s ]/.test(prevChar))
        return true;
    if (/[([{‘“]/.test(prevChar))
        return true; // brackets + smart-open quotes
    return false;
}
                ...
                new Plugin({
                    props: {
                        ...
                        handleTextInput(view, from, to, text) {
                            ...
                            if (!editorPreferences.smartQuotes)
                                return false;
                            ...
                            const prevTwoChars = from > 1 ? state.doc.textBetween(from - 2, from, ' ', ' ') : null;
                            // Quotes
                            if (text === '"') {
...
//# sourceMappingURL=SmartQuotesExtension.js.map