...
function moveByWord(dir, extend) {
    return (state, dispatch) => {
        ...
        const newOffset = dir === 'left' ? findPreviousWordStart(text, offset) : findNextWordStart(text, offset);
        let target;
        if (newOffset !== offset) {
            ...
            const from = state.doc.resolve(Math.min(Math.max($head.pos + bias, 0), state.doc.content.size));
            const near = Selection.findFrom(from, bias, true);
            if (!near || near.$head.pos === $head.pos)
                return false;
...
//# sourceMappingURL=WordNavigationExtension.js.map