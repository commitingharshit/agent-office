 ...
 */
function mergeStylesOntoElement(element, declarations) {
 ...
 */
function inlineStylesFromStyleBlocks(doc) {
 ...
 */
function unwrapGoogleDocsStructuralB(doc) {
 ...
 * folded flat.)
 */
function convertWordLists(doc) {
    var _a, _b, _c;
    const IGNORE_SEL = 'span[style*="mso-list:Ignore"], span[style*="mso-list: Ignore"]';
    const isListPara = (el) => /mso-list\s*:/.test(el.getAttribute('style') || '');
    ...
    while (i < paras.length) {
        ...
        const firstMarker = (((_a = paras[i].querySelector(IGNORE_SEL)) === null || _a === void 0 ? void 0 : _a.textContent) || '').trim();
        const ordered = /[0-9a-z][.)]/i.test(firstMarker);
        const items = [];
        let j = i;
        while (j < paras.length && isListPara(paras[j])) {
            const item = paras[j];
            (_b = item.querySelector(IGNORE_SEL)) === null || _b === void 0 ? void 0 : _b.remove(); // drop the literal bullet/number
            items.push(item);
            j++;
        }
        ...
        (_c = items[0].parentNode) === null || _c === void 0 ? void 0 : _c.insertBefore(list, items[0]);
        for (const item of items)
            item.remove();
        i = j;
    }
...
function transformPastedHTML(html) {
    ...
    if (!hasStyleBlock && !hasGoogleDocsWrapper && !hasWordList)
        return html;
    try {
 ...
 */
function closeListSliceStart(slice) {
...
//# sourceMappingURL=PasteStyleInlinerExtension.js.map