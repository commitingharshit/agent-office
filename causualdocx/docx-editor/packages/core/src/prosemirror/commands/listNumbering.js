 ...
 */
export const restartListNumbering = (state, dispatch) => {
    const { $from } = state.selection;
    const paragraph = $from.parent;
    if (paragraph.type.name !== 'paragraph')
        return false;
    const numPr = paragraph.attrs.numPr;
    if (!numPr || numPr.numId == null)
        return false;
    if (dispatch) {
        const pos = $from.before();
        const tr = state.tr.setNodeMarkup(pos, undefined, Object.assign(Object.assign({}, paragraph.attrs), { listStartOverride: 1 }));
 ...
 */
export const continueListNumbering = (state, dispatch) => {
    const { $from } = state.selection;
    const paragraph = $from.parent;
    if (paragraph.type.name !== 'paragraph')
        return false;
    if (paragraph.attrs.listStartOverride == null)
        return false;
    if (dispatch) {
        const pos = $from.before();
        const tr = state.tr.setNodeMarkup(pos, undefined, Object.assign(Object.assign({}, paragraph.attrs), { listStartOverride: null }));
        dispatch(tr.scrollIntoView());
    }
    return true;
};
//# sourceMappingURL=listNumbering.js.map