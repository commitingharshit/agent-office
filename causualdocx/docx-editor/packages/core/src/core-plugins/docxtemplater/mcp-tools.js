...
Use this to discover what data fields a template document expects.`,
    ...
    handler: (input, context) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            return {
                isError: true,
                content: [
                    { type: 'text', text: `Failed to detect variables: ${error.message}` },
                ],
            };
        }
    }),
 ...
 */
export const insertVariableTool = {
...
Variable names should follow the pattern: letters, numbers, underscores, starting with a letter.`,
    inputSchema: {
        ...
        properties: {
            ...
            variableName: {
                type: 'string',
                description: 'Variable name without braces (e.g., "customer_name"). Will be inserted as {customer_name}',
    ...
    handler: (input, context) => __awaiter(void 0, void 0, void 0, function* () {
        const { documentId, position, variableName } = input;
        const loaded = context.session.documents.get(documentId);
        if (!loaded) {
            return {
                isError: true,
        ...
        try {
            ...
            const { executeCommand } = yield import('../../agent/executor');
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Failed to insert variable: ${error.message}` }],
            };
        }
    }),
    annotations: {
...
Use docx_get_variables first to discover what variables exist in the document.`,
    inputSchema: {
        ...
        properties: {
            ...
            variables: {
                type: 'object',
                description: 'Map of variable names to values (e.g., {"customer_name": "John Doe", "date": "2024-01-15"})',
            ...
            keepUnmatchedVariables: {
                type: 'boolean',
                description: 'If true, keep {variable} placeholders for variables not in the map. If false, replace with empty string. Default: true',
                default: true,
            },
        },
    ...
    handler: (input, context) => __awaiter(void 0, void 0, void 0, function* () {
        const { documentId, variables, keepUnmatchedVariables = true, } = input;
        const loaded = context.session.documents.get(documentId);
        if (!loaded) {
            return {
                isError: true,
        ...
        try {
            ...
            const newDoc = yield parseDocx(result.buffer);
            // Update session
            loaded.document = newDoc;
            ...
            return {
                isError: true,
                content: [{ type: 'text', text: `Failed to apply template: ${error.message}` }],
            };
        }
    }),
    annotations: {
    ...
    handler: (input, context) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        try {
            ...
            return {
                isError: true,
                content: [
                    { type: 'text', text: `Failed to validate template: ${error.message}` },
                ],
            };
        }
    }),
...
// EXPORT ALL TOOLS
// ============================================================================
export const docxtemplaterMcpTools = [
    getVariablesTool,
    insertVariableTool,
    applyTemplateTool,
