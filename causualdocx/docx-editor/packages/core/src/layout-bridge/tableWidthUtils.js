 ...
 * @internal
 */
export function resolveTableWidthPx(value, widthType, parentWidth) {
    if (!value || value <= 0)
        return undefined;
    if (widthType === 'pct') {
...
export function countTableColumns(tableBlock) {
    return Math.max(1, ...tableBlock.rows.map((row) => row.cells.reduce((sum, cell) => { var _a; return sum + Math.max(1, (_a = cell.colSpan) !== null && _a !== void 0 ? _a : 1); }, 0)));
}
/**
 ...
 */
export function normalizeTableColumnWidths(columnWidths, colCount, targetWidth) {
    if (colCount <= 0)
        return [];
    ...
    if (missingColumns > 0) {
        ...
        const fallbackWidth = existingPositive.length > 0
...
//# sourceMappingURL=tableWidthUtils.js.map