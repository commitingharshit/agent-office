 ...
 */
export function findBodyPmSpans(container) {
    return Array.from(container.querySelectorAll(`${BODY_SCOPE} span[data-pm-start][data-pm-end]`));
}
/**
 * All body-tree empty-paragraph runs. Used as a caret fallback when a
 * paragraph has no painted text spans.
 */
export function findBodyEmptyRuns(container) {
    return Array.from(container.querySelectorAll(`${BODY_SCOPE} .layout-empty-run`));
}
/**
 ...
 */
export function findBodyPmAnchors(container) {
    return Array.from(container.querySelectorAll(`${BODY_SCOPE} [data-pm-start]`));
}
/**
 ...
 */
export function findBodyPmAnchor(container, pmStart) {
    if (!Number.isFinite(pmStart))
        return null;
    return container.querySelector(`${BODY_SCOPE} [data-pm-start="${pmStart}"]`);
}
 ...
 */
export function findHeaderFooterPmAnchor(container, pmStart, which) {
    if (!Number.isFinite(pmStart))
        return null;
    const scope = which === 'header' ? HEADER_SCOPE : FOOTER_SCOPE;
    return container.querySelector(`${scope} [data-pm-start="${pmStart}"]`);
}
//# sourceMappingURL=findBodyPmSpans.js.map