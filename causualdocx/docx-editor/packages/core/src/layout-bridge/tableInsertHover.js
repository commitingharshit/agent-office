 ...
 */
export function detectTableInsertHover(input) {
    const { mouseX, mouseY, pagesContainer, target, hfEditMode, edgeProximity = TABLE_INSERT_EDGE_PROXIMITY, } = input;
    ...
    if (tableEl && !tableIsActive(tableEl))
        tableEl = null;
    if (!tableEl) {
    ...
    if (nearLeftEdge) {
        for (let i = 0; i < rows.length; i++) {
            ...
            if (mouseY >= rowRect.top && mouseY <= rowRect.bottom) {
                const cell = rows[i].querySelector(cellSelector);
    ...
    if (nearTopEdge) {
        const cells = rows[0].querySelectorAll(`:scope > ${cellSelector}`);
...
//# sourceMappingURL=tableInsertHover.js.map