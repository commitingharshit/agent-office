 ...
 */
export function clickToPositionDom(container, clientX, clientY, zoom = 1) {
    // Get all elements at the click point
    const elements = document.elementsFromPoint(clientX, clientY);
    // Find the page element
    const pageEl = elements.find((el) => el.classList.contains('layout-page'));
    if (!pageEl)
        return null;
    // Find span with PM position data
    const spanEl = elements.find((el) => el.tagName === 'SPAN' &&
    ...
    const emptyRun = elements.find((el) => el.classList.contains('layout-empty-run'));
    if (emptyRun) {
    ...
    const paragraphEl = elements.find((el) => el.classList.contains('layout-paragraph') && el.dataset.pmStart !== undefined);
    ...
    const cellEl = elements.find((el) => el.classList.contains('layout-table-cell'));
    if (cellEl) {
 ...
 */
function findPositionInSpan(spanEl, clientX, _clientY) {
 ...
 */
function findNearestSpanInElement(element, clientX, clientY) {
    // Check for empty paragraphs within this element
    const emptyRun = element.querySelector('.layout-empty-run');
    if (emptyRun) {
        const paragraph = emptyRun.closest('.layout-paragraph');
        if (paragraph && paragraph.dataset.pmStart) {
            return Number(paragraph.dataset.pmStart);
    ...
    if (!closestLine) {
        ...
        const paragraph = element.querySelector('.layout-paragraph[data-pm-start]');
        if (paragraph === null || paragraph === void 0 ? void 0 : paragraph.dataset.pmStart) {
            return Number(paragraph.dataset.pmStart);
        }
    ...
    if (lineSpans.length === 0) {
        const paragraph = closestLine.closest('.layout-paragraph');
        if (paragraph === null || paragraph === void 0 ? void 0 : paragraph.dataset.pmStart) {
            return Number(paragraph.dataset.pmStart);
        }
        return null;
 ...
 */
function findNearestSpan(_container, pageEl, clientX, clientY, _zoom) {
...
export function getSelectionRectsFromDom(container, from, to, overlayRect) {
    const rects = [];
...
export function getCaretPositionFromDom(container, pmPos, overlayRect) {
    ...
    for (const p of Array.from(paragraphs)) {
        ...
        if (pmPos >= pStart && pmPos <= pEnd) {
            ...
            const pageEl = pEl.closest('.layout-page');
            const pageIndex = pageEl ? Number(pageEl.dataset.pageNumber || 1) - 1 : 0;
            const lineEl = targetEl.closest('.layout-line') || targetEl;
            const lineHeight = lineEl.offsetHeight || 16;
            return {
...
//# sourceMappingURL=clickToPositionDom.js.map