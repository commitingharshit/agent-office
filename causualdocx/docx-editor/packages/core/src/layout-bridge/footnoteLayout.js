 ...
 */
const FOOTNOTE_STYLE_CANDIDATES = ['FootnoteText', 'footnote text'];
/**
 ...
 */
function resolveFootnoteFontSizePt(styles) {
    var _a;
    if (!styles)
    ...
    for (const candidate of FOOTNOTE_STYLE_CANDIDATES) {
        ...
        const halfPoints = (_a = resolved.runFormatting) === null || _a === void 0 ? void 0 : _a.fontSize;
 ...
 */
export function collectFootnoteRefs(blocks) {
    var _a;
    const refs = [];
        ...
        for (const run of block.runs) {
            if (run.kind === 'text' && run.footnoteRefId != null) {
                refs.push({
                    ...
                    pmPos: (_a = run.pmStart) !== null && _a !== void 0 ? _a : 0,
                });
            }
        }
    }
 ...
 */
export function mapFootnotesToPages(pages, footnoteRefs) {
    var _a, _b, _c;
    ...
    for (const ref of footnoteRefs) {
        for (const page of pages) {
            ...
            for (const fragment of page.fragments) {
                const pmStart = (_a = fragment.pmStart) !== null && _a !== void 0 ? _a : -1;
                const pmEnd = (_b = fragment.pmEnd) !== null && _b !== void 0 ? _b : -1;
                if (pmStart >= 0 && pmEnd >= 0 && ref.pmPos >= pmStart && ref.pmPos < pmEnd) {
                    const existing = (_c = pageFootnotes.get(page.number)) !== null && _c !== void 0 ? _c : [];
 ...
 */
export function applyFootnotePresentation(blocks, displayNumber, defaultFontSizePt = DEFAULT_FOOTNOTE_FONT_SIZE_PT) {
    if (blocks.length === 0) {
        return [
            {
                kind: 'paragraph',
    ...
    const out = blocks.map((b) => {
        if (b.kind !== 'paragraph')
            return b;
        const para = b;
        return Object.assign(Object.assign({}, para), { runs: para.runs.map((r) => {
                if (r.kind === 'text' || r.kind === 'tab') {
                    if (r.fontSize == null) {
                        return Object.assign(Object.assign({}, r), { fontSize: defaultFontSizePt });
                    }
                }
                return r;
            }) });
    ...
    if (first.kind === 'paragraph') {
        ...
        out[0] = Object.assign(Object.assign({}, first), { runs: [numberRun, ...first.runs] });
    }
    return out;
}
/**
 ...
 */
export function convertFootnoteToContent(footnote, displayNumber, contentWidth, options) {
    var _a, _b, _c;
    ...
    const rawBlocks = toFlowBlocks(pmDoc, { theme: (_c = options.theme) !== null && _c !== void 0 ? _c : undefined });
 ...
 * Word renders them).
 */
export function buildFootnoteContentMap(footnotes, footnoteRefs, contentWidth, options) {
    ...
    for (const ref of footnoteRefs) {
        ...
        if (!footnote)
            continue;
        contentMap.set(ref.footnoteId, convertFootnoteToContent(footnote, displayNumber, contentWidth, options));
        displayNumber++;
    }
    return contentMap;
}
 ...
 */
export function calculateFootnoteReservedHeights(pageFootnoteMap, footnoteContentMap) {
...
//# sourceMappingURL=footnoteLayout.js.map