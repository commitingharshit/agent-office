 ...
 * an image.
 */
export function hitTestImage(target) {
    if (!(target instanceof Element))
        return null;
    const imageEl = target.closest(IMAGE_HIT_SELECTOR);
    if (!imageEl)
        return null;
 ...
 */
export function captureInlinePositionEmu(imageEl, zoom = 1) {
 ...
 */
export function deriveLayoutChoice(wrapType, cssFloat) {
    if (wrapType === 'inline')
        return 'inline';
    if (wrapType === 'behind')
        return 'behind';
 ...
 */
export function isImageLayoutOptionEnabled(_option, _currentWrapType) {
    return true;
}
// ============================================================================
 ...
 * "no-op".
 */
export function toolbarValueToLayoutTarget(value) {
    switch (value) {
        case 'inline':
            return 'inline';
        case 'square':
