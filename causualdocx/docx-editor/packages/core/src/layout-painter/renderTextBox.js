 ...
 */
export function renderTextBoxFragment(fragment, block, measure, context, options = {}) {
    var _a, _b;
    ...
    const paintedHeight = block.fillColor && fragment.height < 1 ? Math.max(fragment.height, 1) : fragment.height;
    ...
    // Internal padding
    const margins = (_b = block.margins) !== null && _b !== void 0 ? _b : DEFAULT_TEXTBOX_MARGINS;
    ...
    if (block.anchor && !fragment.isAnchored) {
        ...
        const dxPx = isParagraphH && typeof block.anchor.offsetH === 'number'
            ? emuToPixels(block.anchor.offsetH)
            : 0;
        const dyPx = isParagraphV && typeof block.anchor.offsetV === 'number'
    ...
    for (let i = 0; i < block.content.length; i++) {
        ...
        const paraEl = renderParagraphFragment(paraFragment, paraBlock, paraMeasure, Object.assign(Object.assign({}, context), { positioning: 'flow' }), { document: doc });
        containerEl.appendChild(paraEl);
        yOffset += paraMeasure.totalHeight;
    }
    return containerEl;
}
//# sourceMappingURL=renderTextBox.js.map