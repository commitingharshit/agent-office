 ...
 */
function applyRunStyles(element, run, resolvedCommentIds) {
    var _a, _b, _c, _d, _e;
    // Font properties
    if (run.fontFamily) {
    ...
    if (run.kerningMinPt && run.kerningMinPt > 0) {
        const fontSizePt = (_a = run.fontSize) !== null && _a !== void 0 ? _a : 11;
    ...
    if (run.emphasisMark) {
        const variant = run.emphasisMark === 'comma'
            ? 'filled sesame'
        ...
        // Safari prefix.
        element.style.webkitTextEmphasis =
            variant;
        element.style.webkitTextEmphasisPosition = position;
    }
    ...
    if (run.commentIds && run.commentIds.length > 0) {
        const activeCommentId = run.commentIds.find((id) => !resolvedCommentIds || !resolvedCommentIds.has(id));
    ...
    if (run.isInsertion) {
        element.style.backgroundColor = (_b = changeColor === null || changeColor === void 0 ? void 0 : changeColor.bg) !== null && _b !== void 0 ? _b : 'rgba(52, 168, 83, 0.08)';
        element.style.borderBottom = `2px dashed ${(_c = changeColor === null || changeColor === void 0 ? void 0 : changeColor.solid) !== null && _c !== void 0 ? _c : '#2e7d32'}`;
    ...
    if (run.isDeletion) {
        const del = (_d = changeColor === null || changeColor === void 0 ? void 0 : changeColor.solid) !== null && _d !== void 0 ? _d : '#c62828';
        element.style.backgroundColor = (_e = changeColor === null || changeColor === void 0 ? void 0 : changeColor.bg) !== null && _e !== void 0 ? _e : 'rgba(211, 47, 47, 0.08)';
 ...
 */
function applyPmPositions(element, pmStart, pmEnd) {
 ...
 */
function parseMathmlElement(mathml, doc) {
    try {
...
function renderTextRun(run, doc, resolvedCommentIds) {
...
export function renderTabRun(run, doc, width, leader) {
    const span = doc.createElement('span');
    ...
    if (leader && leader !== 'none') {
        ...
        if (leader === 'underscore' || leader === 'heavy') {
            ...
            if (leaderChar) {
                span.style.backgroundImage = `url("data:image/svg+xml,${encodeURIComponent(`<svg xmlns='http://www.w3.org/2000/svg' width='4' height='16'><text x='0' y='12' font-size='12' fill='%23000'>${leaderChar}</text></svg>`)}")`;
 ...
 */
function isUnrenderableImageFormat(src) {
 ...
 */
function unrenderableLabelForSrc(src) {
    const head = src.slice(5, 30).toLowerCase();
    if (head.includes('image/x-emf') || head.includes('image/emf'))
        return 'EMF';
    if (head.includes('image/x-wmf') || head.includes('image/wmf'))
        return 'WMF';
    return 'TIFF';
}
/**
 ...
 * setting display.
 */
function renderImagePlaceholder(run, doc, kind) {
    ...
    const label = run.alt && run.alt.trim().length > 0
        ? `${run.alt} (${fmt})`
 ...
 */
export function renderInlineImageRun(run, doc) {
    ...
    if (run.borderWidth && run.borderWidth > 0) {
        img.style.border = `${run.borderWidth}px ${run.borderStyle || 'solid'} ${run.borderColor || '#000000'}`;
 ...
 */
function renderBlockImage(run, doc) {
    var _a, _b;
    const container = doc.createElement('div');
    container.className = 'layout-block-image';
    container.style.display = 'block';
    container.style.textAlign = 'center';
    container.style.marginTop = `${(_a = run.distTop) !== null && _a !== void 0 ? _a : 6}px`;
    container.style.marginBottom = `${(_b = run.distBottom) !== null && _b !== void 0 ? _b : 6}px`;
    if (isUnrenderableImageFormat(run.src)) {
 ...
 */
function renderImageRun(run, doc) {
 ...
 */
function renderLineBreakRun(run, doc) {
...
function renderFieldRun(run, doc, context) {
    var _a;
    let text = (_a = run.fallback) !== null && _a !== void 0 ? _a : '';
    ...
    return renderTextRun(resolvedRun, doc, context === null || context === void 0 ? void 0 : context.resolvedCommentIds);
}
/**
 * Render a single run (for non-tab runs)
 */
function renderRun(run, doc, context) {
    if (isTextRun(run)) {
        return renderTextRun(run, doc, context === null || context === void 0 ? void 0 : context.resolvedCommentIds);
    }
    if (isTabRun(run)) {
 ...
 */
export function sliceRunsForLine(block, line) {
    const result = [];
    ...
    for (let runIndex = line.fromRun; runIndex <= line.toRun; runIndex++) {
        ...
        if (isTextRun(run)) {
            ...
            if (startChar > 0 || endChar < run.text.length) {
                ...
                result.push(Object.assign(Object.assign({}, run), { text: slicedText, pmStart: run.pmStart !== undefined ? run.pmStart + startChar : undefined, pmEnd: run.pmStart !== undefined ? run.pmStart + endChar : undefined }));
            }
            else {
                result.push(run);
            }
 ...
 */
function getInlineImageRunKey(run) {
    var _a, _b, _c, _d;
    return [
        (_a = run.pmStart) !== null && _a !== void 0 ? _a : 'no-start',
        (_b = run.pmEnd) !== null && _b !== void 0 ? _b : 'no-end',
        run.src,
        run.width,
        run.height,
        (_c = run.displayMode) !== null && _c !== void 0 ? _c : 'inline',
        (_d = run.wrapType) !== null && _d !== void 0 ? _d : 'none',
    ].join('|');
}
/**
 * Convert layout engine TabStop to tab calculator TabStop format
 */
function convertTabStopToCalc(stop) {
    return {
        val: stop.val,
        pos: stop.pos,
        leader: stop.leader,
 ...
 */
function getTextAfterTab(runs, tabRunIndex, context) {
    var _a;
    let text = '';
    for (let i = tabRunIndex + 1; i < runs.length; i++) {
        ...
        if (isTextRun(run)) {
            ...
            if (run.fieldType === 'PAGE' && context) {
                ...
                text += (_a = run.fallback) !== null && _a !== void 0 ? _a : '';
            }
        }
...
function createTextMeasurer(doc) {
    ...
    return (text, fontSize = 11, fontFamily = 'Calibri') => {
        if (!ctx)
            return text.length * 7; // Fallback estimate
 ...
 */
export function renderLine(block, line, alignment, doc, options) {
    ...
    if (runsForLine.length === 1 && isImageRun(runsForLine[0])) {
        const imageRun = runsForLine[0];
        const imageAlign = (_b = (_a = imageRun.position) === null || _a === void 0 ? void 0 : _a.horizontal) === null || _b === void 0 ? void 0 : _b.align;
        const effectiveAlign = imageAlign !== null && imageAlign !== void 0 ? imageAlign : alignment;
    ...
    if (hasTabRuns) {
        ...
        const explicitStops = (_c = options === null || options === void 0 ? void 0 : options.tabStops) === null || _c === void 0 ? void 0 : _c.map(convertTabStopToCalc);
        ...
        const leftIndentTwips = (options === null || options === void 0 ? void 0 : options.leftIndentPx) ? Math.round(options.leftIndentPx * 15) : 0;
        tabContext = {
            explicitStops,
    ...
    let currentX = 0;
    const leftIndentPx = (_d = options === null || options === void 0 ? void 0 : options.leftIndentPx) !== null && _d !== void 0 ? _d : 0;
    if (options === null || options === void 0 ? void 0 : options.isFirstLine) {
        ...
        const firstLineIndentPx = (_e = options === null || options === void 0 ? void 0 : options.firstLineIndentPx) !== null && _e !== void 0 ? _e : 0;
    ...
    for (let i = 0; i < runsForLine.length; i++) {
        ...
        if (isTabRun(run) && tabContext) {
            ...
            const followingText = getTextAfterTab(runsForLine, i, options === null || options === void 0 ? void 0 : options.context);
            ...
            const runEl = renderTextRun(run, doc, (_f = options === null || options === void 0 ? void 0 : options.context) === null || _f === void 0 ? void 0 : _f.resolvedCommentIds);
            ...
            if ((_g = options === null || options === void 0 ? void 0 : options.renderedInlineImageKeys) === null || _g === void 0 ? void 0 : _g.has(imageKey)) {
                continue;
            }
            (_h = options === null || options === void 0 ? void 0 : options.renderedInlineImageKeys) === null || _h === void 0 ? void 0 : _h.add(imageKey);
        ...
        else if (isFieldRun(run) && (options === null || options === void 0 ? void 0 : options.context)) {
            ...
            let fieldText = (_j = run.fallback) !== null && _j !== void 0 ? _j : '';
            ...
            // Fallback for unknown run types
            const runEl = renderRun(run, doc, options === null || options === void 0 ? void 0 : options.context);
            lineEl.appendChild(runEl);
        }
    }
    return lineEl;
 ...
 */
function bordersFormGroup(a, b) {
    if (!a && !b)
        return false; // no borders = no group
    if (!a || !b)
        return false;
    return (bordersEqual(a.top, b.top) &&
 ...
 */
export function renderParagraphFragment(fragment, block, measure, context, options = {}) {
    ...
    const alignment = (_b = block.attrs) === null || _b === void 0 ? void 0 : _b.alignment;
    // Apply paragraph-level styles
    if ((_c = block.attrs) === null || _c === void 0 ? void 0 : _c.styleId) {
    ...
    // Apply RTL direction
    const isBidi = (_d = block.attrs) === null || _d === void 0 ? void 0 : _d.bidi;
    if (isBidi) {
        fragmentEl.dir = 'rtl';
    }
    ...
    const indent = (_e = block.attrs) === null || _e === void 0 ? void 0 : _e.indent;
    let indentLeft = 0;
    let indentRight = 0;
    if (indent) {
    ...
    // Apply borders
    const borders = (_f = block.attrs) === null || _f === void 0 ? void 0 : _f.borders;
    if (borders) {
        ...
        borderBox.style.left = `${indentLeft - ((_h = (_g = borders.left) === null || _g === void 0 ? void 0 : _g.space) !== null && _h !== void 0 ? _h : 0)}px`;
        borderBox.style.right = `${indentRight - ((_k = (_j = borders.right) === null || _j === void 0 ? void 0 : _j.space) !== null && _k !== void 0 ? _k : 0)}px`;
        borderBox.style.top = `${-((_l = renderedTopBorder === null || renderedTopBorder === void 0 ? void 0 : renderedTopBorder.space) !== null && _l !== void 0 ? _l : 0)}px`;
        borderBox.style.bottom = `${-((_m = renderedBottomBorder === null || renderedBottomBorder === void 0 ? void 0 : renderedBottomBorder.space) !== null && _m !== void 0 ? _m : 0)}px`;
    ...
    if ((_o = block.attrs) === null || _o === void 0 ? void 0 : _o.shading) {
    ...
    const paragraphEndsWithLineBreak = (lastRun === null || lastRun === void 0 ? void 0 : lastRun.kind) === 'lineBreak';
    ...
    if ((indent === null || indent === void 0 ? void 0 : indent.hanging) && indent.hanging > 0) {
        firstLineIndentPx = -indent.hanging; // Negative because first line starts further left
    }
    else if ((indent === null || indent === void 0 ? void 0 : indent.firstLine) && indent.firstLine > 0) {
    ...
    const renderedInlineImageKeys = (_p = options.renderedInlineImageKeys) !== null && _p !== void 0 ? _p : new Set();
    for (let i = 0; i < lines.length; i++) {
        ...
        const lineLeftOffset = (_q = line.leftOffset) !== null && _q !== void 0 ? _q : 0;
        const lineRightOffset = (_r = line.rightOffset) !== null && _r !== void 0 ? _r : 0;
        ...
        if (isFirstLine) {
            const hasHangingIndent = (indent === null || indent === void 0 ? void 0 : indent.hanging) && indent.hanging > 0;
            const hasFirstLineIndent = (indent === null || indent === void 0 ? void 0 : indent.firstLine) && indent.firstLine > 0;
            if (hasHangingIndent && (indent === null || indent === void 0 ? void 0 : indent.hanging)) {
                lineAvailableWidth = availableWidth + indent.hanging;
            }
            else if (hasFirstLineIndent && (indent === null || indent === void 0 ? void 0 : indent.firstLine)) {
        ...
        const lineEl = renderLine(block, line, alignment, doc, {
            ...
            tabStops: (_s = block.attrs) === null || _s === void 0 ? void 0 : _s.tabs,
        ...
        const hasHanging = (indent === null || indent === void 0 ? void 0 : indent.hanging) && indent.hanging > 0;
        const hasFirstLine = (indent === null || indent === void 0 ? void 0 : indent.firstLine) && indent.firstLine > 0;
        if (isFirstLine) {
            // First line handling
        ...
        if (isFirstLine && ((_t = block.attrs) === null || _t === void 0 ? void 0 : _t.listMarker) && !((_u = block.attrs) === null || _u === void 0 ? void 0 : _u.listMarkerHidden)) {
            // Override padding for list first lines
            // Marker position = indentLeft - hanging (where first line content starts)
            const markerPos = Math.max(0, indentLeft - ((_v = indent === null || indent === void 0 ? void 0 : indent.hanging) !== null && _v !== void 0 ? _v : 0));
            ...
            const markerFontFamily = (_x = (_w = block.attrs.listMarkerFontFamily) !== null && _w !== void 0 ? _w : firstTextRun === null || firstTextRun === void 0 ? void 0 : firstTextRun.fontFamily) !== null && _x !== void 0 ? _x : block.attrs.defaultFontFamily;
            const markerFontSize = (_z = (_y = block.attrs.listMarkerFontSize) !== null && _y !== void 0 ? _y : firstTextRun === null || firstTextRun === void 0 ? void 0 : firstTextRun.fontSize) !== null && _z !== void 0 ? _z : block.attrs.defaultFontSize;
            const marker = renderListMarker(block.attrs.listMarker, indent, doc, markerFontFamily, markerFontSize);
            ...
            const hanging = (_0 = indent === null || indent === void 0 ? void 0 : indent.hanging) !== null && _0 !== void 0 ? _0 : 0;
            if (hanging > 0) {
 ...
 */
function renderListMarker(marker, indent, doc, fontFamily, fontSize) {
    var _a;
    const span = doc.createElement('span');
    ...
    const hanging = (_a = indent === null || indent === void 0 ? void 0 : indent.hanging) !== null && _a !== void 0 ? _a : 0;
    span.style.display = 'inline-block';
    if (hanging > 0) {
...
//# sourceMappingURL=renderParagraph.js.map