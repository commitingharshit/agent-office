...
function local(name) {
    return (name !== null && name !== void 0 ? name : '').replace(/^.*:/, '').toLowerCase();
}
 ...
 */
export function mathmlToOmml(mathml, options = {}) {
    var _a, _b, _c;
    if (!mathml || !mathml.trim())
        return null;
    let root;
    try {
        const parsed = xml2js(mathml, { compact: false });
        root = ((_a = parsed.elements) !== null && _a !== void 0 ? _a : []).find((e) => e.type === 'element' && local(e.name) === 'math');
        if (!root)
    ...
    if (!body || !/[<]/.test(body))
        return null;
    const rootDisplay = (root.attributes && root.attributes['display']) || 'inline';
    const display = (_c = options.display) !== null && _c !== void 0 ? _c : (rootDisplay === 'block' ? 'block' : 'inline');
    const oMath = `<m:oMath xmlns:m="${OMML_NS}">${body}</m:oMath>`;
    return display === 'block' ? `<m:oMathPara xmlns:m="${OMML_NS}">${oMath}</m:oMathPara>` : oMath;
}
//# sourceMappingURL=mathmlToOmml.js.map