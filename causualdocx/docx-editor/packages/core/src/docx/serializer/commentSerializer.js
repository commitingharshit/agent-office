...
const OOXML_NAMESPACES = 'xmlns:wpc="http://schemas.microsoft.com/office/word/2010/wordprocessingCanvas" ' +
...
function serializeRunContent(run) {
    var _a, _b;
    let xml = '<w:r>';
    // Run properties (minimal — just preserve formatting basics)
    const rPr = [];
    if ((_a = run.formatting) === null || _a === void 0 ? void 0 : _a.bold)
        rPr.push('<w:b/>');
    if ((_b = run.formatting) === null || _b === void 0 ? void 0 : _b.italic)
        rPr.push('<w:i/>');
    if (rPr.length > 0)
...
function serializeParagraph(p, paraId) {
...
function serializeParagraphWithAnnotationRef(p, paraId) {
    let xml = paraId ? `<w:p w14:paraId="${paraId}">` : '<w:p>';
...
function serializeComment(comment, paraInfos) {
 ...
 */
export function serializeCommentsWithInfo(comments) {
 ...
 */
export function serializeCommentsExtended(paraInfos) {
 ...
 */
export function serializeCommentsIds(paraInfos) {
    if (paraInfos.length === 0)
        return '';
    let xml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
 ...
 */
export function serializeCommentsExtensible(paraInfos, comments) {
    if (paraInfos.length === 0)
        return '';
