 ...
 */
function serializeMeasurement(measurement, elementName) {
    if (!measurement)
        return '';
    const attrs = [`w:w="${intAttr(measurement.value)}"`, `w:type="${measurement.type}"`];
 ...
 */
function serializeTableLook(look) {
    if (!look)
        return '';
    const attrs = [];
    if (look.firstRow) {
 ...
 */
export function serializeTableFormatting(formatting, propertyChanges) {
    const parts = [];
    if (formatting) {
...
function extractTblPrInner(tblPrXml) {
...
function serializeTablePropertyChange(change) {
    ...
    const normalizedPreviousTblPr = previousTblPrInner.length > 0 ? `<w:tblPr>${previousTblPrInner}</w:tblPr>` : '<w:tblPr/>';
 ...
 */
export function serializeTableRowFormatting(formatting, propertyChanges, structuralChange) {
    const parts = [];
    if (formatting) {
        ...
        if (formatting.wBefore) {
            parts.push(`<w:wBefore w:w="${intAttr(formatting.wBefore.value)}" w:type="${formatting.wBefore.type}"/>`);
        }
        if (formatting.gridAfter !== undefined) {
            parts.push(`<w:gridAfter w:val="${intAttr(formatting.gridAfter)}"/>`);
        }
        if (formatting.wAfter) {
            parts.push(`<w:wAfter w:w="${intAttr(formatting.wAfter.value)}" w:type="${formatting.wAfter.type}"/>`);
        }
        // Can't split
...
function extractTrPrInner(trPrXml) {
...
function serializeTableRowPropertyChange(change) {
    ...
    const normalizedPreviousTrPr = previousTrPrInner.length > 0 ? `<w:trPr>${previousTrPrInner}</w:trPr>` : '<w:trPr/>';
 ...
 */
function serializeConditionalFormatStyle(style) {
    if (!style)
        return '';
 ...
 */
export function serializeTableCellFormatting(formatting, propertyChanges, structuralChange) {
    const parts = [];
    if (formatting) {
...
function extractTcPrInner(tcPrXml) {
...
function serializeTableCellPropertyChange(change) {
    ...
    const normalizedPreviousTcPr = previousTcPrInner.length > 0 ? `<w:tcPr>${previousTcPrInner}</w:tcPr>` : '<w:tcPr/>';
 ...
 */
export function serializeTableCell(cell) {
    const parts = [];
    // Cell properties
 ...
 */
export function serializeTableRow(row) {
    const parts = [];
    // Row properties
    ...
    if (table.trailingBookmarks) {
        for (const marker of table.trailingBookmarks) {
            if (marker.type === 'bookmarkStart') {
                ...
                parts.push(`<w:bookmarkStart w:id="${marker.id}" w:name="${marker.name}"${colFirst}${colLast}/>`);
            }
            else {
 ...
 */
export function createEmptyTable(rows = 1, cols = 1) {
    const tableRows = [];
 ...
 */
export function createTextCell(text, formatting) {
    return {
        type: 'tableCell',
        formatting,
        content: [
