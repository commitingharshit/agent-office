 ...
 */
import { findChild, findChildren, getAttribute, parseNumericAttribute, } from './xmlParser';
// ============================================================================
// CONSTANTS
// ============================================================================
/**
 ...
 */
export const DEFAULT_TAB_ALIGNMENT = 'left';
/**
 * Default tab leader
 */
export const DEFAULT_TAB_LEADER = 'none';
// ============================================================================
// TAB STOP PARSING
// ============================================================================
/**
 ...
 */
export function parseTabStop(tab) {
 ...
 */
export function parseTabStops(tabs) {
    if (!tabs)
        return [];
 ...
 */
export function parseTabStopsFromParagraphProperties(pPr) {
    if (!pPr)
        return undefined;
 ...
 */
export function mergeTabStops(styleTabs, directTabs) {
    if (!styleTabs && !directTabs)
        return [];
    if (!styleTabs)
        return directTabs !== null && directTabs !== void 0 ? directTabs : [];
    if (!directTabs)
        return [...styleTabs];
 ...
 */
export function getNextTabStop(currentPosition, tabStops, pageWidth) {
    ...
    const defaultPosition = Math.ceil((currentPosition + 1) / DEFAULT_TAB_INTERVAL_TWIPS) * DEFAULT_TAB_INTERVAL_TWIPS;
 ...
 */
export function findTabStopAtPosition(position, tabStops, tolerance = 10) {
    return tabStops.find((tab) => Math.abs(tab.position - position) <= tolerance && tab.alignment !== 'clear');
}
// ============================================================================
 ...
 */
export function calculateTabWidth(currentPosition, tabStops, pageWidth) {
 ...
 */
export function calculateTabWidthWithAlignment(currentPosition, tabStops, pageWidth, followingContentWidth = 0) {
    const nextTab = getNextTabStop(currentPosition, tabStops, pageWidth);
    let width;
 ...
 */
export function getLeaderCharacter(leader) {
    switch (leader) {
        case 'dot':
            return '.';
        case 'hyphen':
 ...
 */
export function hasVisibleLeader(leader) {
 ...
 */
export function generateLeaderString(leader, widthInChars) {
 ...
 */
export function isValidTabAlignment(value) {
 ...
 */
export function isValidTabLeader(value) {
    const validLeaders = ['none', 'dot', 'hyphen', 'underscore', 'heavy', 'middleDot'];
    return validLeaders.includes(value);
}
// ============================================================================
// DEFAULT TAB STOPS
// ============================================================================
 ...
 */
export function generateDefaultTabStops(pageWidth, interval = DEFAULT_TAB_INTERVAL_TWIPS) {
    const tabStops = [];
 ...
 */
export function getEffectiveTabStops(explicitTabs, pageWidth) {
    ...
    let position = Math.ceil((maxExplicitPosition + 1) / DEFAULT_TAB_INTERVAL_TWIPS) * DEFAULT_TAB_INTERVAL_TWIPS;
    while (position < pageWidth) {
        // Only add if there's no explicit tab near this position
        const hasExplicit = explicitTabs.some((t) => Math.abs(t.position - position) < 50 // 50 twips tolerance
        );
        if (!hasExplicit) {
            result.push({
                position,
...
//# sourceMappingURL=tabParser.js.map