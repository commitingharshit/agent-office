 ...
 */
export function executeCommands(doc, commands) {
        ...
        if (item.type === 'run') {
            ...
            if (!inserted && offset >= runStart && offset <= runEnd) {
                ...
                if (insertPos > 0) {
                    ...
                    newContent.push(Object.assign(Object.assign({}, item), { content: [{ type: 'text', text: runText.slice(0, insertPos) }] }));
                }
                // New text
                newContent.push(createTextRun(text, formatting || item.formatting));
                if (insertPos < runText.length) {
                    // Text after insertion point
                    newContent.push(Object.assign(Object.assign({}, item), { content: [{ type: 'text', text: runText.slice(insertPos) }] }));
                }
                inserted = true;
            }
            else {
    ...
    for (const item of paragraph.content) {
        if (item.type === 'run') {
            ...
            if (runEnd <= startOffset || runStart >= endOffset) {
                ...
                if (newText.length > 0) {
                    newContent.push(Object.assign(Object.assign({}, item), { content: [{ type: 'text', text: newText }] }));
                }
            }
            currentOffset = runEnd;
        }
    ...
    for (const item of paragraph.content) {
        if (item.type === 'run') {
            ...
            if (runEnd <= startOffset || runStart >= endOffset) {
                ...
                newContent.push(Object.assign(Object.assign({}, item), { formatting: Object.assign(Object.assign({}, item.formatting), formatting) }));
            }
            else {
                ...
                if (runStart < overlapStart) {
                    newContent.push(Object.assign(Object.assign({}, item), { content: [{ type: 'text', text: runText.slice(0, overlapStart - runStart) }] }));
                }
                // Overlapping text with formatting
                newContent.push(Object.assign(Object.assign({}, item), { formatting: Object.assign(Object.assign({}, item.formatting), formatting), content: [
                        {
                            type: 'text',
                ...
                if (runEnd > overlapEnd) {
                    newContent.push(Object.assign(Object.assign({}, item), { content: [{ type: 'text', text: runText.slice(overlapEnd - runStart) }] }));
                }
            }
            currentOffset = runEnd;
        }
...
function executeInsertText(doc, command) {
    ...
    paragraph.content = insertTextAtOffset(paragraph, command.position.offset, command.text, command.formatting);
    return newDoc;
}
/**
...
function executeReplaceText(doc, command) {
    ...
    if (start.paragraphIndex === end.paragraphIndex) {
        ...
        paragraph.content = insertTextAtOffset(paragraph, start.offset, command.text, command.formatting);
    }
    else {
        ...
        startParagraph.content = insertTextAtOffset(startParagraph, start.offset, command.text, command.formatting);
...
function executeFormatText(doc, command) {
    ...
    if (start.paragraphIndex === end.paragraphIndex) {
        ...
        const paragraph = body.content[blockIndex];
        paragraph.content = applyFormattingInParagraph(paragraph, start.offset, end.offset, command.formatting);
    }
    else {
        // Multiple paragraphs
        for (let i = start.paragraphIndex; i <= end.paragraphIndex; i++) {
            ...
            paragraph.content = applyFormattingInParagraph(paragraph, startOffset, endOffset, command.formatting);
        }
    }
    return newDoc;
}
...
function executeFormatParagraph(doc, command) {
    ...
    const paragraph = body.content[blockIndex];
    paragraph.formatting = Object.assign(Object.assign({}, paragraph.formatting), command.formatting);
    ...
    if ('numPr' in command.formatting) {
        ...
        if (numPr && numPr.numId !== undefined && numPr.numId !== 0) {
            ...
            const ilvl = (_a = numPr.ilvl) !== null && _a !== void 0 ? _a : 0;
            ...
            if (newDoc.package.numbering) {
                ...
                if (num) {
                    const abstractNum = newDoc.package.numbering.abstractNums.find((a) => a.abstractNumId === num.abstractNumId);
                    if (abstractNum) {
...
function executeApplyStyle(doc, command) {
    ...
    const paragraph = body.content[blockIndex];
    paragraph.formatting = Object.assign(Object.assign({}, paragraph.formatting), { styleId: command.styleId });
    return newDoc;
}
/**
...
function executeInsertTable(doc, command) {
    ...
    for (let r = 0; r < command.rows; r++) {
        ...
        for (let c = 0; c < command.columns; c++) {
            const cellText = ((_b = (_a = command.data) === null || _a === void 0 ? void 0 : _a[r]) === null || _b === void 0 ? void 0 : _b[c]) || '';
            cells.push({
                type: 'tableCell',
                content: [
                    {
...
function executeInsertParagraphBreak(doc, command) {
    ...
    const beforeContent = deleteTextInParagraph(Object.assign(Object.assign({}, paragraph), { content: [...paragraph.content] }), command.position.offset, paragraphText.length);
    const afterContent = deleteTextInParagraph(Object.assign(Object.assign({}, paragraph), { content: [...paragraph.content] }), 0, command.position.offset);
