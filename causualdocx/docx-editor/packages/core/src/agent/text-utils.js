 ...
 */
export function getBodyWordCount(body) {
    let count = 0;
    for (const block of body.content) {
        if (block.type === 'paragraph') {
 ...
 */
export function getTableWordCount(table) {
    let count = 0;
    for (const row of table.rows) {
        for (const cell of row.cells) {
            for (const block of cell.content) {
 ...
 */
export function getBodyCharacterCount(body) {
    let count = 0;
    for (const block of body.content) {
        if (block.type === 'paragraph') {
 ...
 */
export function getTableCharacterCount(table) {
    let count = 0;
    for (const row of table.rows) {
        for (const cell of row.cells) {
            for (const block of cell.content) {
 ...
 */
export function getTextBefore(paragraphs, position, maxChars) {
    const texts = [];
    let totalChars = 0;
 ...
 */
export function getTextAfter(paragraphs, position, maxChars) {
    const texts = [];
    let totalChars = 0;
 ...
 */
export function getFormattingAtPosition(paragraph, offset) {
    let currentOffset = 0;
    for (const item of paragraph.content) {
        if (item.type === 'run') {
 ...
 */
export function isPositionInHyperlink(paragraph, offset) {
    let currentOffset = 0;
    for (const item of paragraph.content) {
        if (item.type === 'run') {
            const text = getRunText(item);
 ...
 */
export function getHyperlinkAtPosition(paragraph, offset) {
    let currentOffset = 0;
    for (const item of paragraph.content) {
        if (item.type === 'run') {
            const text = getRunText(item);
 ...
 */
export function isHeadingStyle(styleId) {
    if (!styleId)
        return false;
 ...
 */
export function parseHeadingLevel(styleId) {
    if (!styleId)
        return undefined;
 ...
 */
export function hasHyperlinks(body) {
    for (const block of body.content) {
        if (block.type === 'paragraph') {
            for (const item of block.content) {
 ...
 */
export function hasTables(body) {
 ...
 */
export function getParagraphs(body) {
    return body.content.filter((block) => block.type === 'paragraph');
}
/**
 ...
 */
export function getParagraphAtIndex(body, index) {
 ...
 */
export function getBlockIndexForParagraph(body, paragraphIndex) {
