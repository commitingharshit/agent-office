...
import { fileFormat, type FileFormat } from './format.js';
...
export class S3Host implements HostIntegration {
  ...
  private readonly fmt: FileFormat;

  constructor(private readonly cfg: S3HostConfig) {
    this.fmt = cfg.fmt ?? fileFormat();
  ...
  private keyFor(fileId: FileId): string {
    return `${this.cfg.keyPrefix ?? ''}${fileId}${this.fmt.ext}`;
  }
  ...
  async putFile(
    ...
    await this.send(
      new mod.PutObjectCommand({
        ...
        ContentType: this.fmt.contentType,
        Metadata: {
          'base-file-name': opts.fileName ?? `${fileId}${this.fmt.ext}`,
          version,
        },
      }),
    );
  ...
  async checkFileInfo(fileId: FileId): Promise<FileInfo | null> {
    ...
    try {
      ...
      return {
        baseFileName:
          res.Metadata?.['base-file-name'] ?? `${fileId}${this.fmt.ext}`,
  ...
  async listFiles(): Promise<FileId[]> {
    ...
    do {
      ...
      for (const obj of res.Contents ?? []) {
        ...
        if (!key || !key.endsWith(this.fmt.ext)) continue;
        out.push(key.slice(prefix.length, -this.fmt.ext.length));
      }
