...
export { fileFormat } from './format.js';
...
export async function createHost(): Promise<HostIntegration> {
  ...
  const fmt = fileFormat();

  if (choice === 'memory') {
    return new MemoryHost(fmt);
  }

  if (choice === 'local') {
    const root = process.env.CASUAL_LOCAL_PATH ?? '/data';
    return new LocalHost(root, fmt);
  }
  ...
  if (choice === 'postgres' || choice === 'pg') {
    return new PostgresHost({ url: required('CASUAL_PG_URL'), fmt });
  }
