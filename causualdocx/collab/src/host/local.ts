...
export class LocalHost implements HostIntegration {
  ...
  constructor(root: string, fmt: FileFormat = fileFormat()) {
  ...
  private xlsxPath(fileId: FileId): string {
    return join(this.root, this.safeName(fileId) + this.ext);
  }
  ...
  async putFile(
    ...
    const info: FileInfo = {
      baseFileName: opts.fileName ?? existing?.baseFileName ?? `${fileId}${this.ext}`,
