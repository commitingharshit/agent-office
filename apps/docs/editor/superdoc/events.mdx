...
```javascript
superdoc.on('content-control:active-change', ({ active, previous, source }) => {
...
- **`active` is the primary (deepest) control.** When controls are nested (for
  ...
  control. Use `ui.contentControls.observe()` (which exposes the full active
  stack) when you need the surrounding controls as well.
