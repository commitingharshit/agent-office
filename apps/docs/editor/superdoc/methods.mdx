...
Set the zoom level for all documents and switch the zoom mode to `manual`. Propagates to all presentation editors, PDF viewers, and whiteboard layers. Use `setZoomMode('fit-width')` to re-enter automatic fitting.
...
Switch the zoom mode. `'fit-width'` continuously re-fits the document to the available container width, clamped by [`zoom.fitWidth`](/editor/superdoc/configuration#param-zoom); `'manual'` holds the current value. Switching to `'fit-width'` applies the fit immediately when viewport metrics are available.
...
Get a snapshot of the current zoom state: mode, value, the latest computed fit zoom, and the effective fit bounds.
...
Get the latest viewport measurements: the width available to the document, the document's base page width at 100% zoom, and the unclamped fit zoom. Subscribe to [`viewport-change`](/editor/superdoc/events#viewport-change) for updates.
