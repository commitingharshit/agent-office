...
function setupIPC() {
  ...
  ipcMain.handle(IPC_CHANNELS.getDocumentForTab, (_, tabId) => {
    const payload = getDocumentPayload(tabId);
    return payload ? { success: true, payload } : { success: false };
  });
  
  ipcMain.handle(IPC_CHANNELS.setDocumentMode, (_, { tabId, mode }) => {
    if (Object.values(DOCUMENT_MODE).includes(mode)) {
      const session = setDocumentMode(tabId, mode);
      return { success: true, mode, session };
    }
    return { success: false, error: { code: 'INVALID_MODE', message: `Invalid document mode: ${mode}` } };
  });

  ipcMain.handle(IPC_CHANNELS.markDocumentDirty, (_, tabId) => {
    const session = markSessionDirty(tabId);
    return session ? { success: true, session } : { success: false };
  });
  
  ipcMain.handle(IPC_CHANNELS.saveDocument, async (_, { tabId, bytes }) => {
    try {
      const result = await saveDocumentSession(tabId, bytes);
