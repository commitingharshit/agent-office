...
##### 12. Repaired and hardened Phase 2 SuperDoc embedding
- **Files**:
  - `apps/desktop/document-session.js`
  - `apps/desktop/ipc.js`
  - `apps/desktop/main.js`
  - `apps/desktop/preload.js`
  - `apps/desktop/renderer/index.html`
...
- **Action**:
  - replaced the broken renderer-side package import path with a preload-owned SuperDoc controller
  - switched document transport to byte payloads rather than passing `File`/`Blob` objects over the wrong boundary
  - fixed async session-state usage in the renderer
  - fixed dirty-state tracking so edits, not mode changes, drive save state
  - installed published `superdoc` and `@superdoc-dev/fonts` runtime dependencies for the desktop app
- **Purpose**: Make Phase 2 parse, load, and follow a workable Electron architecture.
