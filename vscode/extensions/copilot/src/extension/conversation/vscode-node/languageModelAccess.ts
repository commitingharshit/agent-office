...
import { HermesGatewayClient } from './hermesBackend';
...
import type { IChatModelInformation } from '../../../platform/endpoint/common/endpointProvider';
...
const hermesBackendMode = (process.env.VSCODE_COPILOT_BACKEND ?? 'hermes').toLowerCase();
const hermesBaseUrl = process.env.VSCODE_HERMES_BASE_URL ?? process.env.HERMES_BASE_URL ?? 'http://127.0.0.1:18789/v1';
const hermesApiKey = process.env.VSCODE_HERMES_API_KEY ?? process.env.HERMES_API_KEY ?? 'hermes-local';
const hermesModelId = process.env.VSCODE_HERMES_MODEL ?? process.env.HERMES_MODEL ?? 'openclaw:main';
const hermesGateway = new HermesGatewayClient(hermesBaseUrl, hermesApiKey);

type HermesModelSpec = {
	id: string;
	name: string;
	family: string;
	isDefault: boolean;
...
const HERMES_MODEL_SPECS: readonly HermesModelSpec[] = [
	{ id: hermesModelId, name: 'Hermes', family: hermesModelId, isDefault: true, isUserSelectable: true },
	{ id: 'gpt-5.5', name: 'Hermes 5.5', family: 'gpt-5.5', isDefault: hermesModelId === 'gpt-5.5', isUserSelectable: true },
	{ id: 'gpt-5.4', name: 'Hermes 5.4', family: 'gpt-5.4', isDefault: hermesModelId === 'gpt-5.4', isUserSelectable: true },
	{ id: 'gpt-5.4-mini', name: 'Hermes 5.4 Mini', family: 'gpt-5.4-mini', isDefault: hermesModelId === 'gpt-5.4-mini', isUserSelectable: true },
];

function createHermesModelName(modelId: string): string {
	switch (modelId) {
		case 'gpt-5.5':
			return 'Hermes 5.5';
		case 'gpt-5.4':
			return 'Hermes 5.4';
		case 'gpt-5.4-mini':
			return 'Hermes 5.4 Mini';
		default:
			return `Hermes ${modelId}`;
	}
}

function createHermesModelSpec(modelId: string, isDefault: boolean): HermesModelSpec {
	return {
		id: modelId,
		name: createHermesModelName(modelId),
		family: modelId,
		isDefault,
...
function isHermesBackendEnabled(): boolean {
	return hermesBackendMode !== 'copilot';
}

function createHermesModelInformation(spec: HermesModelSpec): IChatModelInformation {
	return {
		id: spec.id,
		vendor: 'hermes',
		name: spec.name,
		model_picker_enabled: true,
		is_chat_default: spec.isDefault,
		is_chat_fallback: false,
		version: 'local',
		urlOrRequestMetadata: hermesBaseUrl,
		capabilities: {
			type: 'chat',
			family: spec.family,
...
function createHermesLanguageModelInfo(spec: HermesModelSpec): vscode.LanguageModelChatInformation {
	const info = createHermesModelInformation(spec);
	return {
		id: info.id,
		name: info.name,
		family: info.capabilities.family,
		version: info.version,
		maxInputTokens: info.capabilities.limits?.max_prompt_tokens ?? 128_000,
		maxOutputTokens: info.capabilities.limits?.max_output_tokens ?? 8_192,
		capabilities: {
			imageInput: false,
			toolCalling: true,
		},
		tooltip: `Hermes local backend at ${hermesBaseUrl} using ${spec.id}`,
		isUserSelectable: spec.isUserSelectable,
		isDefault: {
			[ApiChatLocation.Panel]: spec.isDefault,
			[ApiChatLocation.Terminal]: spec.isDefault,
			[ApiChatLocation.Notebook]: spec.isDefault,
			[ApiChatLocation.Editor]: spec.isDefault,
		},
	} as unknown as vscode.LanguageModelChatInformation;
}

function getHermesModelSpec(modelId: string): HermesModelSpec {
	return HERMES_MODEL_SPECS.find(spec => spec.id === modelId) ?? HERMES_MODEL_SPECS[0];
}

function createHermesEndpoint(modelId: string, instantiationService: IInstantiationService): OpenAIEndpoint {
	const spec = getHermesModelSpec(modelId);
	return instantiationService.createInstance(
		OpenAIEndpoint,
		createHermesModelInformation(spec),
		hermesApiKey,
		hermesBaseUrl,
	);
}
...
export class LanguageModelAccess extends Disposable implements IExtensionContribution {
	...
	private async _registerChatProvider(): Promise<void> {
		...
		this._register(this._authenticationService.onDidAuthenticationChange(() => {
			if (!isHermesBackendEnabled() && !this._authenticationService.anyGitHubSession) {
				this._currentModels = [];
			}
	...
	private async _provideLanguageModelChatInfo(options: { silent: boolean }, token: vscode.CancellationToken): Promise<vscode.LanguageModelChatInformation[]> {
		if (isHermesBackendEnabled()) {
			const supportedModelIds = await hermesGateway.getSupportedModelIds(token);
			const modelIds = supportedModelIds.length > 0 ? supportedModelIds : HERMES_MODEL_SPECS.map(spec => spec.id);
			const defaultModelId = modelIds.includes(hermesModelId) ? hermesModelId : modelIds[0];
			const models = modelIds.map(modelId => createHermesLanguageModelInfo(createHermesModelSpec(modelId, modelId === defaultModelId)));
	...
	private async _getEndpointForModel(model: vscode.LanguageModelChatInformation) {
		if (isHermesBackendEnabled()) {
			return createHermesEndpoint(model.id, this._instantiationService);
		}
...
export class CopilotLanguageModelWrapper extends Disposable {
	...
	private async _provideLanguageModelResponse(_endpoint: IChatEndpoint, _messages: Array<vscode.LanguageModelChatMessage | vscode.LanguageModelChatMessage2>, _options: vscode.ProvideLanguageModelChatResponseOptions, extensionId: string | undefined, callback: FinishedCallback, token: vscode.CancellationToken): Promise<APIUsage | undefined> {
		...
		if (isHermesBackendEnabled()) {
			const result = await hermesGateway.complete(_endpoint.model, messages, _options.modelOptions as Omit<OptionalChatRequestParams, 'n'> | undefined, token);
			await callback(result.text, 0, { text: result.text });
			return result.usage;
		}
