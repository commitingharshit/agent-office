...
function getElectron(arch: string): () => NodeJS.ReadWriteStream {
	return () => {
		const cachedZip = getCachedElectronZip(arch);
		const repo = cachedZip ? async ({ fileName }: { url: string; fileName: string }): Promise<Response> => {
			const expectedZipName = `electron-v${electronVersion}-${process.platform}-${arch}.zip`;
			if (fileName === 'SHASUMS256.txt') {
				const checksums = fs.readFileSync(path.join(root, 'build', 'checksums', 'electron.txt'), 'utf8');
				return new Response(checksums, {
					status: 200,
					headers: { 'Content-Length': String(Buffer.byteLength(checksums)) }
				});
			}
			...
			const size = (await fs.promises.stat(cachedZip)).size;
			const body = Readable.toWeb(fs.createReadStream(cachedZip)) as ReadableStream<Uint8Array>;
			return new Response(body, { status: 200, headers: { 'Content-Length': String(size) } });
		} : config.repo;
...
function getCachedElectronZip(arch: string): string | undefined {
...
async function main(arch: string = process.arch): Promise<void> {
	...
	const executableName = process.platform === 'win32' ? `${product.applicationName}.exe` : product.applicationName;
	const executablePath = path.join(electronPath, executableName);

	// Prefer a local Electron cache so launches can work offline.
	if (fs.existsSync(versionFile) && fs.existsSync(executablePath)) {
		const cachedVersion = fs.readFileSync(versionFile, 'utf8').trim();
		if (cachedVersion === electronVersion) {
			return;
		}
	}
